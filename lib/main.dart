...
// ============================================================
// VOICE ROOM CONFIG
const String kAgoraTokenServer = 'http://162.35.161.192:3000';
...
class _RoomPageState extends State<RoomPage> {
  ...
  Future<void> _startVoice() async {
    ...
    try {
      ...
      if (!ok) {
        ...
        final step = _voice.lastStep;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            duration: const Duration(seconds: 10),
            content: Text('فشل عند خطوة: $step\nAgora: $error'),
          ),
        );
      }
    } catch (e) {
  ...
  void _showVoiceConfigMessage() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('تعذر تشغيل الصوت حالياً، تحقق من إعدادات Agora.'),
...
class _AgoraVoiceController {
  ...
  Future<bool> init({
    ...
    try {
      lastStep = 'طلب صلاحية المايكروفون';
      ...
      lastStep = 'طلب التوكن من السيرفر';
      ...
      lastStep = 'إنشاء الـ Engine';
      debugPrint('AGORA: creating engine');

      final e = createAgoraRtcEngine();

      lastStep = 'تهيئة الـ Engine (initialize)';
      await e.initialize(
      ...
      lastStep = 'تفعيل الصوت (enableAudio)';
      await e.enableAudio();

      debugPrint('AGORA: audio enabled');

      lastStep = 'الانضمام للقناة (joinChannel)';
      await e.joinChannel(
        token: token,
      ...
      try {
        ...
        debugPrint('AGORA WARN: setEnableSpeakerphone failed: $spkErr');
        // لا نوقف الجلسة بسبب فشل تفعيل السماعة، الصوت يشتغل بالسماعة الافتراضية.
      }
  ...
  Future<void> setMic(bool enabled) async {
    ...
    try {
      ...
      debugPrint('AGORA WARN: muteLocalAudioStream failed: $err');
      // لا نرمي الخطأ حتى لا ينهار كامل الجلسة بسبب مشكلة مؤقتة بالتوقيت.
    }
  }
