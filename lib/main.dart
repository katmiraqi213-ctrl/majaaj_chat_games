...
class _RoomPageState extends State<RoomPage> {
  ...
  Future<void> _toggleMic() async {
    ...
    try {
      final next = !_onMic;
      await _voice.setMic(next);
      if (!mounted) return;
      setState(() {
        _onMic = next;
        if (!next) _micMuted = false;
      });
      unawaited(_membersRef.doc(FirebaseAuth.instance.currentUser?.uid).set({'onMic': next, 'muted': _micMuted}, SetOptions(merge: true)));

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(next ? '🎙️ صعدت على المايك' : '🔇 نزلت من المايك'),
  ...
  Widget _micCard(Map<String, dynamic> mic, int index) {
    final role = mic['role'] as String;
    final roleColor = _roleColor(role);
    final bool isMe = index == 0 && _onMic;
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: isMe ? Colors.greenAccent : roleColor,
                    width: isMe ? 3 : 2,
                  ),
                ),
                child: Icon(
                  mic['icon'] as IconData,
                  color: roleColor,
                  size: 30,
                ),
              ),
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: ListView.separated(
                    ...
                    itemCount: _mics.length,
                    separatorBuilder: (_, __) => const SizedBox(width: 8),
                    itemBuilder: (context, index) =>
                        _micCard(_mics[index], index),
                  ),
                ),
