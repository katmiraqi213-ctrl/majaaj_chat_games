  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: const Color(0xFF9B5CFF).withValues(alpha: .30), blurRadius: 22)],
      ),
      child: Center(
...
class GamesPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text('الألعاب', style: TextStyle(fontWeight: FontWeight.w900)), actions: [IconButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const GameHistoryPage())), icon: const Icon(Icons.history))]), body: ListView(padding: const EdgeInsets.all(16), children: [
    const _SectionTitle(title: 'ألعاب مزاج', subtitle: 'اللعب أونلاين • نقاط • ترتيب'), const SizedBox(height: 12),
    _GameCard(icon: Icons.grid_4x4_rounded, title: 'لودو', subtitle: '2 أو 4 لاعبين', badge: 'جاهزة', enabled: false, onTap: () {}),
    _GameCard(icon: Icons.style_rounded, title: 'أونو', subtitle: 'بطاقات وتحديات', badge: 'قريباً', onTap: () {}),
    _GameCard(icon: Icons.radio_button_checked_rounded, title: 'كيرم', subtitle: 'تنافس أونلاين', badge: 'قريباً', onTap: () {}),
    _GameCard(icon: Icons.close_rounded, title: 'XO', subtitle: '1 ضد 1', badge: 'قريباً', onTap: () {}),
  ]);
}

class _GameCard extends StatelessWidget {
  final IconData icon; final String title, subtitle, badge; final VoidCallback onTap; final bool enabled;
  const _GameCard({required this.icon, required this.title, required this.subtitle, required this.badge, required this.onTap, this.enabled=true});
  @override Widget build(BuildContext context) => Card(child: InkWell(onTap: enabled ? onTap : null, borderRadius: BorderRadius.circular(20), child: Padding(padding: const EdgeInsets.all(16), child: Row(children: [Container(width:60,height:60,decoration:BoxDecoration(borderRadius:BorderRadius.circular(18), color:Theme.of(context).colorScheme.primary.withValues(alpha:.14)), child: Icon(icon,size:31)), const SizedBox(width:14), Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(title,style:const TextStyle(fontSize:18,fontWeight:FontWeight.bold)),const SizedBox(height:4),Text(subtitle,style:const TextStyle(color:Colors.white54))])), Container(padding:const EdgeInsets.symmetric(horizontal:10,vertical:6),decoration:BoxDecoration(borderRadius:BorderRadius.circular(20),color:enabled?Colors.green.withValues(alpha:.15):Colors.white.withValues(alpha:.06)),child:Text(badge,style:TextStyle(fontSize:12,color:enabled?Colors.greenAccent:Colors.white54,fontWeight:FontWeight.bold)))]))));
}

class WalletPage extends StatelessWidget {
  const WalletPage({super.key});
  @override
  Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text('المحفظة', style: TextStyle(fontWeight: FontWeight.w900)), actions: [IconButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const WalletHistoryPage())), icon: const Icon(Icons.receipt_long_outlined))]), body: StreamBuilder<DocumentSnapshot<Map<String,dynamic>>>(stream: FirebaseFirestore.instance.collection('users').doc(FirebaseAuth.instance.currentUser!.uid).snapshots(), builder:(context,snap){
    final data=snap.data?.data()??{}; final balance=(data['walletBalance'] as num?)?.toDouble()??0;
    return ListView(padding:const EdgeInsets.all(16),children:[
      Container(padding:const EdgeInsets.all(22),decoration:BoxDecoration(borderRadius:BorderRadius.circular(26),gradient:const LinearGradient(colors:[Color(0xFF4C1D95),Color(0xFFBE185D)])),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Text('رصيد المحفظة',style:TextStyle(color:Colors.white70)),const SizedBox(height:8),Text('${balance.toStringAsFixed(0)} نقطة',style:const TextStyle(fontSize:32,fontWeight:FontWeight.w900)),const SizedBox(height:18),Row(children:[Expanded(child:FilledButton.icon(onPressed:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>const TopUpPage())),icon:const Icon(Icons.add),label:const Text('شحن'))),const SizedBox(width:10),Expanded(child:OutlinedButton.icon(onPressed:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>const WalletHistoryPage())),icon:const Icon(Icons.history),label:const Text('الحركات')))])]),),
      const SizedBox(height:18),const _SectionTitle(title:'طرق الشحن',subtitle:'نفعّل الطرق المتاحة بعد ربط بوابة الدفع'),const SizedBox(height:8),
      const _InfoTile(icon:Icons.credit_card,title:'بطاقات الدفع',subtitle:'ماستر كارد / فيزا / البطاقات المدعومة'),const _InfoTile(icon:Icons.phone_android,title:'رصيد الهاتف',subtitle:'شحن عن طريق رصيد الشبكة حسب توفر الخدمة'),const _InfoTile(icon:Icons.admin_panel_settings_outlined,title:'شحن يدوي',subtitle:'طلب إلى الإدارة ومراجعة العملية'),
    ]);
  });
}
